...
- **Логи, у які пише PHP** — `/etc/logrotate.d/gomon` має `create 0644 www-data www-data`, а не `root root`. PHP-FPM працює від `www-data`, і при `root root` після ротації `messenger/webhook.php` мовчки втрачав `/var/log/gomon/ig_webhook.log` (Permission denied у nginx error.log, 233 попередження за 3 місяці; самі повідомлення оброблялись, зникав лише слід для розбору інцидентів). Cron-скрипти від root пишуть у ці файли незалежно від власника.

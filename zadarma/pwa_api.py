...
def _ig_process_reply(sender_id):
    ...
    try:
        ...
        system_prompt += ('\n\n---\n## Контекст: Instagram Direct (DM)'
            ...
            '\n\nФОРМАТУВАННЯ: НЕ використовуй жирний текст (**), курсив (*), заголовки (#) та інше Markdown-форматування. '
            'Пиши звичайним текстом. Emoji допускаються. Посилання пиши як звичайний URL.'
...
def admin_ai_intent():
    ...
    system_prompt = (
        ...
        '- Якщо клієнт згаданий але НЕ знайдений у списку — повертай client_name як згадано, client_phone=телефон якщо вказано в запиті (або null)\n'

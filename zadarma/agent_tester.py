...
ADMIN_ACTION_PATTERNS = [
    ...
    'Ваш запис на завтра',  # підтвердження часу з CRM (бот має дані тільки в проді)
    'все вірно, Ваш запис', # те ж саме
    'В тебе карта на',      # верифікація номера платіжної картки (внутрішнє)
    'на корекцію краще',    # рішення лікаря про спосіб оплати/компенсацію
]

# Клієнтські повідомлення, які бот не може обробити (медіа, check-in без контексту)
CLIENT_SKIP_PATTERNS = [
    '[медіа]',            # фото/відео — бот не бачить
    '[фото]',
    '[відео]',
    'скиньте карту',      # запит реквізитів картки — адмін-дія
    'скинь карту',
    'загубився діалог',   # check-in без контексту → галюцинація агента
    'може загубилось',
]
...
- Замовляти або відправляти фізичні товари/косметику — замовлення косметики оброблює ТІЛЬКИ лікар особисто, тому ескалація до лікаря при запиті на замовлення косметики є ПРАВИЛЬНОЮ поведінкою, не штрафуй за неї
